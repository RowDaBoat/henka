...
proc toAlias *(conv :var Converter; cursor :CXCursor; name :string) :cint=
  ...
  if underlying.kind == CXType_Elaborated:
    ...
    if   elabName.startsWith("struct ") : elabName = elabName[7..^1]
    elif elabName.startsWith("enum ")   : elabName = elabName[5..^1]
    if elabName == name: return CXChildVisit_Continue.cint
...
proc toObject *(conv :var Converter; cursor :CXCursor; name :string) :cint=
  ...
  let structName = if isTagged: conv.addRenamed(StructType, name)
                   else:        conv.addRenamed(Typedef, name)
  # Collect fields
...
proc toEnum *(conv :var Converter; cursor :CXCursor; name :string) :cint=
  if name.len == 0: return CXChildVisit_Continue.cint
  if name in conv.seenEnums: return CXChildVisit_Continue.cint
...
proc toVariable *(conv :var Converter; cursor :CXCursor; name :string) :cint=
  ...
  if isConst: typeStr = typeStr[6..^1]
...
proc toMacro *(conv :var Converter; cursor :CXCursor; name :string) :cint=
  if name.startsWith("_") or name.startsWith("__"): return CXChildVisit_Continue.cint
  if clang_Cursor_isMacroFunctionLike(cursor) != 0: return CXChildVisit_Continue.cint
