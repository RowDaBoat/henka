...
proc add_comment*(conv: var Converter, cursor: CXCursor): Option[astTF.Id]=
  ...
  if text.startsWith("///"):
    text = text[3..^1].strip(leading = true, trailing = false)
  elif text.startsWith("//"):
    text = text[2..^1].strip(leading = true, trailing = false)
  elif text.startsWith("/**") and text.endsWith("*/"):
    text = strip_block_comment(text[3..^3])
  elif text.startsWith("/*")  and text.endsWith("*/"):
    text = strip_block_comment(text[2..^3])
